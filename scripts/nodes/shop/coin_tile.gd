...
func icon_texture() -> Texture2D:
	var icon := get_node_or_null("Panel/Content/IconCircle/Icon") as TextureRect
	return icon.texture if icon != null else null


func _refresh() -> void:
	var color := Color(str(item_data.get("color", "#F59E0B")))
	var bar := get_node_or_null("Panel/Bar") as ColorRect
	if bar != null:
		bar.color = color
	# Texture vòng icon đã khai trong `coin_tile.tscn` — ở đây chỉ tô màu theo gói
	var circle := get_node_or_null("Panel/Content/IconCircle") as TextureRect
	if circle != null:
		circle.self_modulate = Color(color.r, color.g, color.b, 0.18)
	var icon := get_node_or_null("Panel/Content/IconCircle/Icon") as TextureRect
	if icon != null:
...
func _set_texts() -> void:
	...
	_set_label("Panel/Content/Description/Name", TranslationServer.translate(str(item_data.get("name_key", ""))))
	_set_label("Panel/Content/Description/Desc", desc)
	# Dòng ưu đãi: chỉ hiện khi MÔ TẢ chưa nhắc tới số Xu thưởng (tránh lặp 2 dòng giống nhau)
	var bonus := int(item_data.get("bonus", 0))
	var bonus_label := get_node_or_null("Panel/Content/Description/Bonus") as Label
	if bonus_label != null:
...
func _set_button() -> void:
	# Art nút (`btn_coin.svg`) + kiểu chữ giá VNĐ đã khai sẵn trong `coin_tile.tscn`
	# (cả 4 trạng thái) — ở đây chỉ điền số tiền của gói.
