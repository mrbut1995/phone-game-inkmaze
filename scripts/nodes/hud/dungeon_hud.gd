...
func _on_update(ctx: Dictionary) -> void:
	set_label_text(get_node_or_null("Step/Value"), str(maxi(int(ctx.get("steps_remaining", 0)), 0)))
	set_label_text(get_node_or_null("Floor/Value"), "%02d" % maxi(int(ctx.get("floor_number", 1)), 1))
