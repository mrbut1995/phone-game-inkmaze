...
const SEGMENT_SCENE := preload("res://nodes/hud/portrait/game/countdown_segment.tscn")
...
func _on_update(ctx: Dictionary) -> void:
	...
	set_label_text(get_node_or_null("Content/ModeInformation/Sheet/Budget/Value"), "%02d" % left)
	set_label_text(get_node_or_null("Content/ModeInformation/Sheet/Budget/Max"), "/ %d" % total)
	set_label_text(get_node_or_null("Content/ModeInformation/Sheet/Spent/Value"), "-%02d" % spent)
	set_label_text(get_node_or_null("Content/ModeInformation/Sheet/Spent/Note"),
		tr("STR_HUD_BUDGET_CELLS").format([int(ctx.get("moves", 0))]))
	set_label_text(get_node_or_null("Content/ModeInformation/Sheet/Price/Reserve"),
...
func _update_price_chips(mode: CountdownCostGameMode) -> void:
	...
	var cheap := get_node_or_null("Content/ModeInformation/Sheet/Price/ChipCheap")
	if cheap != null:
		set_label_text(cheap.get_node_or_null("Label"),
			tr("STR_HUD_PRICE_CHEAP").format([range_v.x, mini(range_v.x + 1, range_v.y)]))
	var pricey := get_node_or_null("Content/ModeInformation/Sheet/Price/ChipPricey") as Control
	if pricey == null:
		return
...
func _update_segments(total: int, spent: int) -> void:
	var box := get_node_or_null("Content/ModeInformation/Sheet/Segments") as HBoxContainer
	if box == null:
		return
