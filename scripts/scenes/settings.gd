...
## CÙNG tên node). Các node đã BIND SẴN bằng `@export` trong `scenes/layout/<hướng>/settings.tscn`
## ⇒ code đọc qua `layout.<tên>`, KHÔNG tra đường dẫn; thêm/đổi node chỉ cần sửa scene + export.
var layout: SettingsLayout = null
...
func _bind_refs() -> void:
	layout = active_layout() as SettingsLayout
	if layout == null:
		push_warning("settings: bố cục chưa gắn SettingsLayout — thiếu binding trong scenes/layout/<hướng>/settings.tscn")
...
func _setup_debug_stamp_taps() -> void:
	if layout.lbl_version == null:
		return
	var stamp := layout.lbl_version.get_parent() as Control
	if stamp == null:
		return
...
func _sync_from_settings() -> void:
	...
	var sm := _sm()
	if sm != null:
		if layout.bgm_slider != null:
			layout.bgm_slider.value = float(sm.get("music_volume"))
		if layout.sfx_slider != null:
			layout.sfx_slider.value = float(sm.get("sfx_volume"))
		if layout.chk_haptic != null:
			layout.chk_haptic.button_pressed = bool(sm.get("vibration"))
		if layout.chk_auto_mark != null:
			layout.chk_auto_mark.button_pressed = bool(sm.get("auto_mark_safe"))
		if layout.chk_glow != null:
			layout.chk_glow.button_pressed = bool(sm.get("glow_path"))
...
func _update_volume_labels() -> void:
	if layout.bgm_value != null and layout.bgm_slider != null:
		layout.bgm_value.text = _to_percent(layout.bgm_slider.value)
	if layout.sfx_value != null and layout.sfx_slider != null:
		layout.sfx_value.text = _to_percent(layout.sfx_slider.value)
...
func _refresh_language() -> void:
	if layout.lbl_language != null:
		layout.lbl_language.text = Loc.display_name(Loc.current())
...
func _refresh_version() -> void:
	if layout.lbl_version != null:
		...
		layout.lbl_version.text = tr("STR_SETTINGS_VERSION").format([version])
	if layout.lbl_player_id != null:
		layout.lbl_player_id.text = PLAYER_ID_PLACEHOLDER
