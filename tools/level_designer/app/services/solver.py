...
CHỈ đi được trong các ô THUỘC BOARD (cell_mask) - ô trống ngoài board coi như tường.
"""
...
def neighbors(level: LevelModel, cell: Cell) -> list[Cell]:
    """Các ô đi được từ `cell` (đã tính tường chặn + ô thuộc board)."""
    x, y = cell
    result: list[Cell] = []
    ...
    if not level.has_wall(("h", x, y)):
        result.append((x, y - 1))
    return [c for c in result if level.is_cell_active(c)]
...
def analyze(level: LevelModel) -> dict:
    ...
    Mọi thống kê tính trên SỐ Ô THUỘC BOARD (không tính ô trống ngoài board).
    """
    board_cells = level.active_count()
    empty_cells = level.width * level.height - board_cells
    base = {
        "solved": False,
        "path": None,
        "path_length": 0,
        "reachable_cells": 0,
        "total_cells": board_cells,
        "board_cells": board_cells,
        "empty_cells": empty_cells,
        "blocked_cells": board_cells,
        "slack": 0,
        "all_reachable": False,
    }
    if not level.start_end_ok():
    ...
    return {
        ...
        "board_cells": board_cells,
        "empty_cells": empty_cells,
        "blocked_cells": board_cells - len(dist),
        "slack": level.max_steps - path_length,
        "all_reachable": len(dist) == board_cells,
    }
